var orderofpcburl = rootUrl + "OrderManage/OrderShareListData?random=" + Math.floor(Math.random() * (100000 + 1));//请求地址
var detailurl = rootUrl + "OrderManage/OrderDetailInfo?random=" + Math.floor(Math.random() * (100000 + 1));//请求地址
var delurl = rootUrl + "OrderManage/ShareDelOrder?random=" + Math.floor(Math.random() * (100000 + 1));//请求地址
...
var savesendgoodsurl = rootUrl + "OrderManage/ShareProductSendGoods?random=" + Math.floor(Math.random() * (100000 + 1));//请求地址
var returngoodsurl = rootUrl + "OrderManage/ShareProductReturnGoods?random=" + Math.floor(Math.random() * (100000 + 1));//请求地址
var count1 = 1;
var count1max;
var pagetype = "ordershareproduction";
$(document).ready(function () {
    requestOrderListData();
    $(".search-ordershareproduction-data").on("click", requestOrderListData);//收藏
  ...
  **查看当前共享生产的详细信息
  **/
    $('.search-shareproductiondetailinfo').on('click', function () {

        var shareorderid = GetCheckedValues("orderid");
        if (shareorderid == "" || shareorderid == null) {
            alert("请选择需要查看的订单!")
            return false;
        }

        requestDetailInfo(shareorderid);
        $(".basicorderinfo").css("display", "none");
        $(".orderofshareproductioninfo").css("display", "block");

    })
    /******
    ***删除当前共享生产的订单
    ********/
    $('.del-thisorderofshareproduction').on('click', function () {
...
function requestOrderListData() {
    ...
    postdata["Order_phone"] = $("#order_phone").val();
        ...
        success: function (data) {
            ...
            for (var i = 0; i < jsonRecords.length; i++) {
                ...
                $("#table_list").append(
                                ...
                                //    "<td><button type='button'  data-shareproducorderid='" + jsonRecords[i]['ORDER_ID'] + "' class='btn btn-primary search-shareproducorderdetailinfo'>查看</button></td>" +
                                    //    "<td><button type='button' data-shareproducorderid='" + jsonRecords[i]['ORDER_ID'] + "' class='btn btn-danger del-thisorderofshareproduc'>删除</button></td>" +
                                "</tr>"
                            );
...
function backpreinfo() {
    ...
    $(".orderofshareproductioninfo").css("display", "none");
